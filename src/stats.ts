...
export function normal(mean: number, sd: number, allowNegative: boolean): number {
  ...
  if (!allowNegative && mean < 0 && mean + sd * 2.5 < 0)
    throw new RangeError(
      'Negative means must be within 2 standard deviations of 0 if negative results are not allowed.'
    );
...
export function generateNormalLabTest(
  ...
  if (_.has(overriddenTests, labTest.id)) {
    // Lab test is overridden, compute the new mean/sd values.
