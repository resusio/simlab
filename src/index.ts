...
import { orderLabTestsByDependency, orderLabTestsByInverseDependency } from './dependencyGraph';
...
class LabReportGenerator {
  ...
  public fetchLabReport({ labIds, units = 'metric' }: { labIds?: string[], units?: string } = {}) {
    ...
    const labTestIdsComputed = labIds ? _.intersection(labIds, this.requestedLabTestResultIds) : this.requestedLabTestResultIds;
  ...
  public updateSingleTest(testId: string, newValue?: testResultType): string[] {
    ...
    if (newValue) {
      ...
      const newResult = this.computeSingleLab(labTestConfig, this.fullLabReportResults, this.getDiseaseOverriddenTests(this.diseases), this.patient);
      this.fullLabReportResults[testId] = newResult;
    }

    if (labTestConfig.generate.neededBy) {
      const neededByResults = _.flatten(labTestConfig.generate.neededBy.map((testId) => [...this.updateSingleTest(testId)]));
  ...
  public updateAndFetchSingleTest(testId: string, newValue?: testResultType, units: string = 'metric'): fullTestResultType {
