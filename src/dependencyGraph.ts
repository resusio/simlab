 ...
 */
function createDependencyNode(lab: labTestType, requestedLabs: labTestType[], labTestDb: labTestType[]): DependencyNode {
  ...
  if (lab.generate.method === labTestGenerateMethod.DERIVED) {
    if (lab.generate.requires) {
      ...
      lab.generate.requires.forEach((requiredLabId) => {
        ...
        const requiredLabConfig = labTestDb.find((checkLabTest) => checkLabTest.id === requiredLabId);
 ...
 */
export function orderLabTestsByDependency(requestedLabTests: labTestType[], labTestDb: labTestType[]): labTestType[] {
